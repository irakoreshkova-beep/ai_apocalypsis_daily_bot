...
FEEDS = [
    ...
    {"name": "AI News", "kind": "rss", "url": "https://www.artificialintelligence-news.com/feed/"},
]
...
def save_state(config: Config, items: list[NewsItem], *, mark_sent: bool = False, punchline: str = "") -> None:
    state = load_state()
    existing = state.get("sent_fingerprints", [])
    merged = existing + ([item.fingerprint for item in items] if config.mode == "rubric" else [])
    recent_punchlines = state.get("recent_punchlines", [])
    if punchline:
        compact_punchline = re.sub(r"\s+", " ", punchline).strip()
        if compact_punchline:
            recent_punchlines = [line for line in recent_punchlines if line != compact_punchline]
            recent_punchlines.append(compact_punchline)
    payload = {
        **state,
        "sent_fingerprints": merged[-config.state_limit :],
        "recent_punchlines": recent_punchlines[-50:],
...
def build_model_input(items: list[NewsItem], bonus_rubric: str) -> str:
    ...
    payload = {
        ...
        "mode": os.getenv("DIGEST_MODE", "brief").lower(),
        "recent_punchlines": state.get("recent_punchlines", [])[-30:],
        "items": [
            {
                "source": item.source,
                "title": item.title,
...
def format_bonus_post(parsed: dict) -> str:
    ...
    source_title = re.sub(r"\s+", " ", bonus.get("source_title", "")).strip()
    source_url = bonus.get("source_url", "").strip()
    body = bonus.get("body", [])
    why_it_matters = re.sub(r"\s+", " ", bonus.get("why_it_matters", "")).strip()
    reality_check = re.sub(r"\s+", " ", bonus.get("reality_check", "")).strip()
    if not rubric or not title or not body:
        return ""

    lines = [f"<b>{html.escape(rubric)} // {html.escape(title)}</b>"]
    if source_title and source_url:
        lines.extend(["", f"<b>Источник:</b> <a href=\"{html.escape(source_url, quote=True)}\">{html.escape(source_title)}</a>"])
    elif source_title:
        lines.extend(["", f"<b>Источник:</b> {html.escape(source_title)}"])
    for paragraph in body[:4]:
        text = re.sub(r"\s+", " ", paragraph).strip()
        if text:
            lines.extend(["", html.escape(text)])
    if why_it_matters:
        lines.extend(["", f"<b>Почему это важно:</b> {html.escape(why_it_matters)}"])
    if reality_check:
        lines.extend(["", f"<b>Без хайпа:</b> {html.escape(reality_check)}"])
...
def format_analysis_post(parsed: dict) -> str:
    ...
    source_title = re.sub(r"\s+", " ", analysis.get("source_title", "")).strip()
    source_url = analysis.get("source_url", "").strip()
    ...
    body = json.dumps(
        {
            ...
            "instructions": (
                ...
                "В intro не используй болезни, рак, пациентов, смерть или трагедии как шутку или лёгкий образ. Если есть медицинская новость, упомяни её нейтрально. "
                "Closing оставь пустым, если нет очень короткой смешной финальной фразы. "
                "В humanity_status дай один короткий острый панчлайн. Это не summary и не дополнительный вывод. "
                "Выбери ровно один самый смешной крючок из выпуска: абсурдная идея, публичная фигура, название модели, название компании, цена, рынок, запрет, странный продукт или очевидный конфликт реальности с хайпом. "
                "Не пытайся упомянуть несколько новостей сразу. Не перечисляй Apple + OpenAI + Musk в одной шутке. Один крючок — один удар. "
                "Панчлайн должен быть максимум 110 символов или две очень короткие фразы. Лучше короче. "
                "Лучший тип шутки: конкретная картинка или поворот, как 'Маск вынес дата-центры в космос, потому что на Земле скептики слишком близко'. "
                "Ищи крючки в названиях моделей, компаний, продуктов и организаций: Claude, ChatGPT, Gemini, Grok, Llama, Sora, Midjourney, Runway, OpenAI, Anthropic, Google, Meta, xAI, Nvidia, Adobe, Figma и похожие. "
                "Если название даёт естественную игру слов или каламбур, используй его. Например: Grok как грубоватый пророк из X, Claude как слишком вежливый убийца рутины, Gemini как близнец, который спорит сам с собой, Sora как киноотдел апокалипсиса. "
                "Каламбур должен быть понятным без объяснений и не должен ломать факты новости. Если игра слов натянутая, выбери абсурдный публичный крючок вместо каламбура. "
                "Можно иронизировать над публичными персонажами AI-индустрии и техно-бизнеса: Сэм Альтман, Илон Маск, Дженсен Хуанг, Марк Цукерберг, Big Tech, инвесторы, акции, презентации, вечные обещания AGI. "
                "Не шути над болезнями, пациентами, трагедиями, смертью и конкретными непубличными людьми. Если в выпуске есть медицина или рак, не делай это объектом шутки; выбери другой крючок. "
                "Не выдумывай факты о людях и компаниях. Не делай универсальную шутку, которую можно вставить в любой выпуск. Если панчлайн не связан с сегодняшними новостями, перепиши. "
                "Не повторяй и не перефразируй recent_punchlines из входных данных. Запрещены шаблоны про 'отнял работу', 'кофе и дедлайны', 'люди всё ещё главные', 'технологии в кошельках и головах'. "
                "Плохой формат: пересказ выпуска. Хороший формат: короткий удар по одному крючку. "
                ...
                "analysis_post должен опираться на одну конкретную новость из входных items. Обязательно заполни source_title и source_url из выбранной новости. "
                "Если выбранная новость важна только в связке с другими, всё равно укажи главный источник, от которого отталкиваешься. "
                ...
                "В analysis_post не добавляй humanity_status и не делай отдельную шутку в конце: достаточно поля reality_check. Если mode не analysis, верни analysis_post.enabled=false и пустые строки во всех остальных полях analysis_post. "
                ...
                "bonus_post должен опираться на одну конкретную новость из входных items. Обязательно заполни source_title и source_url из выбранной новости. "
                "Не делай общие рассуждения без источника. Если нет подходящей новости, поставь enabled=false. "
                "Если bonus_rubric = SIGNAL, сделай короткий сигнал дня: какая конкретная новость показывает сдвиг в культуре, дизайне, профессиях, small business или интернете. "
                "Если bonus_rubric = CREATIVE AI RADAR, выбери новость про генеративную графику, видео, дизайн, креативные workflows, медиа, авторские права или creative tools и объясни, почему это важно креативщикам. "
                "Если bonus_rubric = WEIRD AI WATCH, выбери конкретную странную, мемную или абсурдную AI-новость: баги моделей, странные генерации, синтетический интернет, ботов, дипфейки, AI-спам, нелепые автоматизации, продукты с ощущением 'зачем это вообще сделали'. "
                "WEIRD AI WATCH должен быть забавным и наблюдательным: покажи, что именно странно, почему это смешно или тревожно-смешно, и какой кусок реальности тут поехал. Не превращай это в мрачную колонку и не морализируй. "
                "Если прямой странной новости нет, можно взять обычную новость и найти в ней weird-угол только если он реально следует из фактов. Иначе поставь enabled=false. "
                "Если bonus_rubric = DESIGN TOOLS WATCH, выбери новость про AI-инструменты для дизайнеров, визуала, видео, презентаций, интерфейсов, контента или креативной автоматизации. Если прямого инструмента нет, честно возьми ближайшую новость и объясни, как она может повлиять на дизайн-работу. "
                "Если bonus_rubric = FUTURE JOBS / EXTINCT JOBS, выбери новость, которая меняет задачи или профессии: что исчезает, что остаётся человеческим, какие новые задачи появляются. "
                "bonus_post должен быть коротким: 2-3 абзаца, живым языком, без списка инструментов и без советов 'как пользоваться'. why_it_matters — конкретный вывод, reality_check — трезвое ограничение без хайпа. "
...
def extract_punchline(parsed: dict, config: Config) -> str:
    if config.mode != "brief":
        return ""
    humanity_status = re.sub(r"\s+", " ", parsed.get("humanity_status", "")).strip()
    return re.sub(r"^HUMANITY STATUS:\s*", "", humanity_status, flags=re.IGNORECASE)
...
def main() -> None:
    ...
    if items or mark_sent:
        save_state(config, items, mark_sent=mark_sent, punchline=extract_punchline(parsed, config))
