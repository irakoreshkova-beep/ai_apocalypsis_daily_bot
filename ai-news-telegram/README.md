...
- `cron-job.org` запускает GitHub workflow каждый день в `11:30` по Тбилиси
- скрипт защищен от дублей и отправляет не больше одного cron-дайджеста в день
- если свежих новостей нет, сообщение в канал не отправляется
- отдельные рубрики запускаются отдельным cron-job: `SIGNAL` по понедельникам, `DEAD INTERNET REPORT` по средам, `FUTURE JOBS / EXTINCT JOBS` по пятницам
